...
/** A V4L2 driver for ov5640 camera modules.
 ...
 NOTE: v4l2_mbus_pixelcode is depreciated (linus torvald) and  MEDIA_BUS_FMT_ definitions in media-bus-format.h should
be used. For our parallel interface there is only one MediaBusFormat: V4L2_MBUS_PARALLEL (parallel interface with hsync
and vsync) We use the Video Format as FOURCC code to be reported to the driver. It tells how video data is coded and how
many bits per pixel are used. The data then passes through the CSI module.

 The CSI Module:
 Data is captured into a fifo-buffer. Its format can be RAW(any Camera format) or YUV422(to be optionally converted by
CSI). Data can be optionally converted before it leaves the CSI_module out to the memory. Possible output conversions
are: PassThrough = no conversion. mostly used. CSI-Input must be set to RAW! When CSI-input is set to YUV422(default),
the following conversions are possible:
 ...
 Data is then transferred to the memory via special CSI-internal DMA, and an Interrupt is generated at the end of a full
frame dma-transfer. For this, the fifos need a destination memory address (buffer) to place the data of the frame. (This
is not a kernel DMA!) Double buffering is possible which uses alternating fifos/buffers. (read while other is written
to!)
 ...
 Depending on the Videoformat one Pixel has a specific Bitlength (depth) of information. common values are 8, 12 or 16
bits per pixel. So the max. Pixel per line is reduced to: 4096*8/Bitlength); for 8bit=4096; for 12bit=2730, for
16bit=2048 (pixel per line)
 ...
 * @xfer_func:  transfer function of the data (from enum v4l2_xfer_func)
 ...
 Common Frame Sizes and Framerates:
  at 24MHZ input clock to the camera, the outputted pixelclock is fe. 48MHZ, thus pumping out 48-million Bytes per
second.!! At two Bytes per Pixel, we can pump out 24 million pixel per second. Format      Framesize/Pixel   Framerate
(24-million / Framesize*2 = Frames per Second)
  =====================================
 320 * 240    76800        156
 640 * 480    307200       39
 1280 * 720   921600       13
...
#define csi_debug(l, x, arg...) \
    if (l >= DBG_LEV) printk(KERN_CRIT "[-----OV5640]" x, ##arg)
#else
#define csi_debug(l, x, arg...)
#endif
...
static struct regval_list regs_init_VGA[] =  // default: VGA 640x480, 4:3, 15fps, subsample On, Scaling On
    {
        ...
        {{0x30, 0x0e}, {0x58}},  // DVP enable, (default:0x58): 0x58 = 0101 1000 = mipi off, mipiPhy=off;
                                 // mipiPhy=powerdown, DVP parallel port=0n
        ...
        {{0x3c, 0x01}, {0x00}},  // b4,34// default:0x00=off.0x34=0n.AutoBanding[7], Sum auto mode enable[5], Band
                                 // counter enable[4], Counter threshold[3-0] for band change = 4
        ...
        {{0x35, 0x03}, {0x00}},  // AEC AGC auto enable(0). AECauto[0], AGCauto[1] default: 0x00 = both enabled
                                 // nightmode+banding:
        {{0x3a, 0x00},
         {0x78}},  // AEC Band Night: BandFunctionOn[5], NightModeOn[2] default:0x78=BandingOn. 0x7C=nightmode
        //- BandFunctionOn(3a00_5) = manual Bandfilter on(1), if on, aec is done in steps of "band steps". if off, aec
        //is continuoues.
        //                            0x3c01[7] must be 1=manual, and the relevant 50 or 60 hz frequency must be
        //                            selected in same register.

        {{0x3a, 0x05},
         {0x30}},  // NightModeInsertFrames[6] , exposureStepAuto[5],step auto-ratio can be set in bits 0-4.default:0x30

        {{0x3a, 0x17},
         {0x03}},  //[1:0]   Gain night threshold = gain value from which nightmode will be enabled (auto-nightmode)
        ...
        {{0x3a, 0x08}, {0x01}},  // 16bit 50hz band step width, is set approx to VTS automaticly, maybe need to set
                                 // extra framelines as well.
        ...
        {{0x3a, 0x1b},
         {47}},  // 48.control Range High Limit (go out), control region . control should be bigger than stable
        {{0x3a, 0x1e}, {35}},  // 38.control Range Low Limit (go out) if yavg within this range, no change, else will
                               // adjust as to stable range.
        ...
        {{0x3a, 0x19}, {0xf8}},  // AGC gain ceiling[7:0]  the higher the limit, the lower the exposure time can be.
                                 // medium values are good.
        ...
        {{0x30, 0x34},
         {0x1a}},  //(6-4)=charge pump loop filter. (3-0)=BITdivider. 1a=10bit(default), 18=8bit(a bit faster!)
        ...
        {{0x31, 0x08},
         {0x01}},  //(5-4)=PCLK Rootdivider=0;(3-2)=sclk2x root divider=dontcare; (1-0)=SCLK root divider; default:0x16.
        ...
        // ISP: Lens Control register, lens correction values:	(these are custom values supplied by omnivision for
        // specific customer, so take as is)
...
static struct regval_list
    regs_QSXGA_FULL_2592_1944[] =  // qsxga: 2592*1944, pclk:28m, 4:3, 5 fps, subsample Off, Scaling Off
    {
        {{0x30, 0x08}, {0x42}},  // stop streaming during register setting
...
static struct regval_list
    regs_HD1080_1920_1080[] =  // 1080: 1920*1080 , pclk:42m, 16:9, 15 fps, subsample Off, Scaling Off
    {
        {{0x30, 0x08}, {0x42}},  // stop streaming during register setting
...
static struct sensor_format_struct {
    ...
    int bitsperpixel;  // for this format. (bytesperline). if csi conversion, bytesperline equals the max plane (Y) =
                       // width!
    ...
    enum csi_seq_e byte_order;         // the byte order of the color of input, ie. YUYV, YVYU, UYVY,VYUY
                                       // camera init data:
    struct regval_list
        *regs;      // the cameras videoformat. for csi conversion always yuv422. i2c init array to intialyze a format
...
static int sensor_read(struct v4l2_subdev *sd, unsigned char *reg, unsigned char *value)
{
    ...
    for (i = 0; i < REG_ADDR_STEP; i++) data[i] = reg[i];  // copy address bytes

    for (i = REG_ADDR_STEP; i < REG_STEP; i++) data[i] = 0xff;  // append 0xff as filler, if any
    /*
    ...
    ret = i2c_transfer(client->adapter, &msg, 1);
    if (ret >= 0) {
        for (i = 0; i < REG_DATA_STEP; i++) value[i] = data[i + REG_ADDR_STEP];
        ret = 0;
    } else {
...
static int sensor_write(struct v4l2_subdev *sd, unsigned char *reg, unsigned char *value)
{
    ...
    for (i = 0; i < REG_ADDR_STEP; i++) data[i] = reg[i];
    for (i = REG_ADDR_STEP; i < REG_STEP; i++) data[i] = value[i - REG_ADDR_STEP];
...
static ulong get_pclk(struct v4l2_subdev *sd)
{
    ...
    csi_debug(
        2,
        "Predivider = %d,Multiplier = %d,SystemClockDivider = %d,Rootdivider = %d,BITdivider = %d,PCLKdivider = %d\n",
...
static int calc_fps(struct v4l2_subdev *sd)
{
    ...
    //------------------	get output size before scaling, as double offset from isp origin, or offset of origin within
    //isp window
    ...
    if ((hts && (vts + vts_extra)) == 0) return 0;
    pclk = get_pclk(sd);
    if (!pclk) {
    ...
    csi_debug(2, "IspWidth:%u IspHeight:%u PoutWidht:%u PoutHeight:%u Outwidth;%u OutHeight:%u\n", ispwidth, ispheight,
              preoutwidth, preoutheight, outwidth, outheight);
...
settings info:
agc aec: exposure(3500-02),
gain(350a-b),luminance(56a1),extra(350c-d),aec(3503),agc,ashigh(3a0f),aslow(3a10),achigh(3a1b),aclow(3a1e) (exta-lines
only in nightmode or manual aec!) aec-average-lumina enable manual and size: aw,xstart,xend,ystart,yend); aec submodes
50/60Hz banding abf: bandon(3aA00_5),bandauto(3c01_7),bandfreq(3C0C_0) aec nightmode: nighton(3aA00_2),
frameinsert(3A05_5), nightgain(3A17_0,1), max_expos60(3A02,3A03),max_expos50(3A14,3A15),
...
void showinfo(void)
{
    ...
    unsigned aw, xstart, xend, ystart, yend, bandon, bandauto, bandf50, nighton, frameinsert, nightgain, max_expo60,
        max_expo50, minexpo;
    ...
    csi_debug(3, "--->Exposure:%lu Gain:%u Lum:%u extraL:%u aec:%u agc:%u ash:%u asl:%u ach:%u acl:%u\n", exposure,
              gain, luminance, extra, aec, agc, ashigh, aslow, achigh, aclow);
    ...
    csi_debug(3, "- nighton:%u frameinsert:%u nightgain:%u max_expo60:%u max_expo50:%u minexpo:%u\n", nighton,
              frameinsert, nightgain, max_expo60, max_expo50, minexpo);
...
int scale_image(unsigned int outwidth, unsigned int outheight)
{
    ...
    ispy = 1944;
    xoffset =
        16;  // the default offset for above full frame. these def values mask out the non visilbe sensor rows/colomns
    yoffset = 4;
...
int zoom_image(unsigned int outwidth, unsigned int outheight)
{
    ...
    if ((outwidth < maxx) || (outheight < maxy))  // if output size is smaller than biggest possible
    {
        ...
        if (ispx < 1422)  // experiment shows, values under 1422 do not work, because of analog sensor readout timing as
                          // limitation
            ispx = 1422;  // min limit.
...
static int sensor_try_fmt_internal(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt,
                                   struct sensor_format_struct **ppformat, struct sensor_win_size **ppsize)
{
    int index;
    ...
    for (index = 0; index < ARRAY_SIZE(sensor_formats); index++)
        if (sensor_formats[index].fourcc == fmt->code) break;
    ...
    for (psize = sensor_win_sizes; psize < sensor_win_sizes + (ARRAY_SIZE(sensor_win_sizes)); psize++)
        if (fmt->width >= psize->width && fmt->height >= psize->height) break;

    // if no match found, take the smallest one, the last
    if (psize >= sensor_win_sizes + (ARRAY_SIZE(sensor_win_sizes))) psize--;

    // set the found pointers as info for the caller
    if (ppformat != NULL) *ppformat = sensor_formats + index;
    if (ppsize != NULL) *ppsize = psize;
...
{
    if (index >= ARRAY_SIZE(sensor_formats)) return -EINVAL;
...
static int sensor_enum_framesizes(struct v4l2_subdev *sd, struct v4l2_frmsizeenum *fsize)
{
    ...
    if ((fsize == NULL) || (fsize->index >= ARRAY_SIZE(sensor_win_sizes)))
        return -EINVAL;  // ARRAY_SIZE=number of entrys
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt)
{
    ...
    // we call try-fmt again here(user should have done that before), just to make sure!, also we get the pointers we
    // need.
    ret = sensor_try_fmt_internal(sd, fmt, &pformat, &psize);
    if (ret) return ret;  // no match found
...
static int sensor_detect(struct v4l2_subdev *sd)
{
    ...
    if (regs.value[0] != 0x56) return -ENODEV;
...
static int camera_init(struct v4l2_subdev *sd, u32 val)
{
    ...
    // here we init the camera with all values needed for all formats. format specific stuff is transfered on top of
    // this.
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
    ...
    case CSI_SUBDEV_STBY_ON:
        ...
        //       gpio_write_one_pin_value(pcam->csi_pin_hd,CSI_STBY_ON,csi_stby_str); // in
        //       \linux-sunxi-lemaker-3.4\arch\arm\plat-sunxi/sys_config.c
        msleep(10);
        break;
    case CSI_SUBDEV_STBY_OFF:
        csi_debug(2, "CSI_SUBDEV_STBY_OFF\n");
        //       gpio_write_one_pin_value(pcam->csi_pin_hd,CSI_STBY_OFF,csi_stby_str); // no i2c possible if standby!!.
        //       so we leave it on.
        msleep(10);
        break;
...
Block diagram:
                                                                                      ...
                                                                                      |
XVCLK->Predivider->Phase detector->Loop filter(chargepump)->VCO->System clock divider->
Rootdivider->BITdivider->PCLKdivider->PCLK |                                             |
...
Register 3800 - 3807: ISP Window
     ...
     visible area = ISP Input Window. - this area is fed into the image sensor processor ISP. the smaller the faster
FPS! using downscaling: if ISP-Window has same aspect ratio as sensor. just define your output size in DVPHO and DVPVO
to be downscaled. if ISP-Window aspect ratio different to sensor: Need to correct ISP-window-hight to match output
aspect ratio: Conversion Factor CF = DVPHO / DVPVO; ie. Width/Height define ISP Input Image Height IH= fe. 1944. then
ISP Input Image Width IW = IH * CF . example for full image: Xoffset = 16; no change Yoffset = 4 +(2592  - IW)/2
...
Here one line is composed of 1600pixelclock (active picture)(HREF=ON), and 322pixelclock(horizontal blanking)
(HREF=OFF). The blanking is unclear if needed, but at least defines the offtime of the HREF pulse(and thus the time for
one line!. So the total horizontal pixelclocks are 1922. Depending on your PLL and timing settings, you get a certain
Pixelclock frequency. Lets assume we have a pixelclock of 36Mhz. Then Trow = 39,39usecs. (one line) The total Frametime
results in 66,62msecs (if no additional dummy-lines are added, to adjust Framerate!!). So the rollingshutter
exposuretimes are: minimum=39,39usecs and maximum=66,62msecs.
...
ov5640 supports larger than one frame exposuretimes in registers 0x350C/0x350D !! which adds dummylines to the
frame.manual only
...
DVP = Digital Video Port, 8Bit parallel video output port, as opposed to MIPIport. performs mjeg, CCIR656, hsync pulse
if wanted.

Alltogether 3 modes of sensor picturesize change(output size/data conversion)  are available:
- subsampling/binning = full image but only use every second or third pixel for output data.2x2 binnig reduce picsize by
4! or is it 2?
...
+ 2: OUTPUT SIZE: define the DVP output picture size that is transmitted out on the parallel port:
    ...
    If no scaling is used, it should match +4. otherwise it crops +4 from its origin. Or if bigger creates a subarea in
big window.
    {{0x38,0x08},{0x02}},            //output width from x-start = 280 = 640    DVPHO (default 2592)  = active (high)
portion of HREF
    {{0x38,0x09},{0x80}},            //
...
+ 3: TIMING: This defines the Video Timing, Linetime in Pixelclocks, Frametime in Lines. HREF-period, VSYNC Period.
    It includes non-visible pixels at start/end of sensor and dummy-lines, and blanking. values should be dividable
by 4. It must not be less than the ISP Inputwindow! or if binning, half of it. i guess? It also controls exposure and
sensor readout logic. Its actually used by the ISC to generate HREF and VSYNC. (caution: the CSI interface can only
handle 4096 Bytes, thus for yuv422 thats 2048!maybe?)
    {{0x38,0x0c},{0x07}}, //total width = 768 = 1896   HTS (default 2844)  Horizontal total size for 1 line = HREF
Period
    {{0x38,0x0d},{0x68}}, //
    {{0x38,0x0e},{0x03}}, //total height = 3d8 = 984   VTS (default 1968)  Vertical total size for 1 frame. VTS*HTS =
VSYNC Period
    {{0x38,0x0f},{0xd8}}, //

+ 4: DIGITAL CROB: This defines a Subwindow in the ISP Inputwindow.  It defines the size of the output-window if no
scaling is used. ie. it crops. If scaling is used, this Subwindow is used to be downscaled to the outputsize as defined
in +2. So we have actually 2 methods of "cropping" the physical sensor: +1  and +4. where only +1 will have effect on
Framerate! The offsets are applied to the beginning and end of the ISP Window to result in an always centered window
asto +1. Therefore the offset values must be divided by 2. effetively this commonly is used to remove the dummy lines
and rows, or adjusting aspect ratio!
    {{0x38,0x10},{0x00}},             //isp x-offset = 10 = 16  H offset(default 16) default min offset = valid array
size
    {{0x38,0x11},{0x10}},              //
...
binning means, averaging the lightlevels of adjacent pixel (the ones that are skipped by subsample) into the subsample
destination. Thus considering also the skipped pixels,giving brigter picture.
...
defining the average window for YAVG, the picture average luminance value in 0x56a1:(only needed if manual enabled in
0x501D)!! 0x5680 - 01 = 12bit X-start . Horizontal start position for average window 0x5684 - 85 = 12bit X-end 0x5682 -
03 = 11bit Y-start Vertical start position for average window 0x5686 - 87 = 11bit Y-end

gain(analog gain):
automatic gain control will only activate if aec control reaches its limit. Value is autoset in autoagc!
0x350a-0b = 10bit current analog gain value (auto set in auto agc). maximum gain is 64. range:0-1023. so div 16 gives
real gain. 0x3A18-19 = 10bit AGC ceiling/max value

+++ nightmode:
0x3A00[2] = nightmode enable(1)
0x3A05[6] =  insert frame enable(1). increase and decrease step based on frames(1) or lines/bands(0).off=longer
exposure! 0x3A05[5] =  exposure step auto(1) or manual(0). step auto-ratio can be set in bits 0-4 0x3A17[1:0]   Gain
night threshold = gain value when nightmode gets activated night mode ceiling:(max exposure when nightmode is used)
