...
#define define_fbtft_write_reg(func, buffer_type, data_type, modifier)                                                \
    ...
    {                                                                                                                 \
        ...
        *buf = modifier((data_type)va_arg(args, unsigned int));                                                       \
        ...
        len--;                                                                                                        \
                                                                                                                      \
        if (par->startbyte) *(u8 *)par->buf = par->startbyte | 0x2;                                                   \
                                                                                                                      \
        if (len) {                                                                                                    \
            i = len;                                                                                                  \
            while (i--) *buf++ = modifier((data_type)va_arg(args, unsigned int));                                     \
            fbtft_write_buf_dc(par, par->buf, len * (sizeof(data_type) + offset), 1);                                 \
        }                                                                                                             \
    out:                                                                                                              \
        va_end(args);                                                                                                 \
    }                                                                                                                 \
            ...
            void fbtft_write_reg8_bus9(struct fbtft_par *par, int len, ...)
{
    ...
    if (unlikely(par->debug & DEBUG_WRITE_REGISTER)) {
        va_start(args, len);
        for (i = 0; i < len; i++) *(((u8 *)buf) + i) = (u8)va_arg(args, unsigned int);
        va_end(args);
...
int fbtft_write_vmem16_bus8(struct fbtft_par *par, size_t offset, size_t len)
{
    ...
    /* non buffered write */
    if (!par->txbuf.buf) return par->fbtftops.write(par, vmem16, len);
    ...
    while (remain) {
        ...
        for (i = 0; i < to_copy; i++) txbuf16[i] = cpu_to_be16(vmem16[i]);
...
#else
        for (i = 0; i < to_copy; i++) txbuf16[i] = 0x0100 | vmem8[i];
#endif
        vmem8 = vmem8 + to_copy;
